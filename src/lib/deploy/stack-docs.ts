  ...
  parts.push(
    ...
    dbs.length > 0
      ? `Databases in this stack (${dbs.join(', ')}) refuse to start without a password. ` +
          'BuildMyStack generates one for each and inlines it into the compose (mirrored in ' +
...
export function buildStackDocs(stack: PersistedStack, options: StackDocsOptions = {}): string {
  ...
  // Title + overview.
  parts.push(`# ${name}`, '');
  parts.push(`Generated by BuildMyStack. Services: ${serviceNames}.`, '');
  parts.push('## Overview', '');
  ...
  if (secretKeys.length > 0) {
    parts.push(
      `BuildMyStack generated ${secretKeys.length} secret${secretKeys.length === 1 ? '' : 's'} for this stack ` +
        '(e.g. database passwords). They are inlined in `docker-compose.yml` and mirrored in `.env`.',
      '',
    );
  } else {
    parts.push(
      'Any passwords BuildMyStack generated for you are inlined in `docker-compose.yml` and ' +
        'mirrored in `.env`.',
      '',
    );
  }
