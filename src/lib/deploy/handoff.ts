/**
 * Deployment HANDOFF — Stapelwerk's complementary value vs Coolify/Dokploy.
 *
 * Stapelwerk is the guided *composer*; it does not compete with a deploy
...
export interface ComposeBundle {
   ...
   * server over THEIR own SSH. Stapelwerk never sees the server or the key.
   */
  deployScript: string;
 ...
 * Stapelwerk**. It `scp`s the self-contained compose (next to the script) to
...
# Stapelwerk — key-sovereign deploy.
#
# Runs on YOUR machine and drives YOUR server over YOUR own SSH. Stapelwerk
...
export function buildEnvFile(secrets: Record<string, string>): string {
  const lines = [
    '# Generated by Stapelwerk — auto-generated passwords for your stack.',
