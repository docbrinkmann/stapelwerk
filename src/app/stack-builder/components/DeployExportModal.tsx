 ...
 * deliverables: a self-contained docker-compose.yml, a reference .env, and a
 * README — plus per-target instructions (Portainer / Coolify / Dokploy / plain
 * docker compose) and environment guides (Raspberry Pi / Home Server / VPS).
 *
 * The compose/env/secrets are passed in already-generated so the modal shows
 * the exact same passwords as the live preview (no re-randomization).
 */
export function DeployExportModal({
  ...
  const targets = useMemo(() => getDeployTargets(), []);
  const [guideEnv, setGuideEnv] = useState<DeploymentEnv>('raspberry-pi');
  const guide = useMemo(() => getDeploymentGuide(guideEnv, stack), [guideEnv, stack]);
  const secretEntries = Object.entries(secrets);

  if (!isOpen) return null;

  const readme = buildReadme(stack, renderGuideMarkdown(guide), secrets);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Deploy or export stack"
      onClick={onClose}
    >
      <div
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              Run your stack anywhere — portable compose plus step-by-step instructions.
            </p>
          </div>
        ...
        <div className="min-h-0 flex-1 space-y-6 overflow-y-auto p-6">
          ...
          <div className="rounded-lg border border-border bg-card">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
              <div className="text-sm font-medium">Environment guide</div>
              <div className="flex flex-wrap gap-1">
                {DEPLOYMENT_ENVS.map(({ id, label }) => {
...
function InlineCode({ value }: { value: string }) {
  return (
    <div className="flex items-start justify-between gap-2 rounded-md border border-border bg-muted/50 px-3 py-2">
      ...
      <CopyButton value={value} label="Copy command" iconOnly />
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-warning/40 bg-warning/10 p-4">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <CopyButton value={envFile} label="Copy .env" />
      </div>
      <p className="mb-3 text-xs text-muted-foreground">
        These are inlined in the compose and mirrored in the downloadable .env. Store them
        somewhere safe — you’ll need them to access your services.
      </p>
      <ul className="space-y-1.5">
        {secrets.map(([key, value]) => (
          ...
          >
            ...
            <CopyButton value={value} label="Copy value" iconOnly />
          </li>
        ))}
      </ul>
    </div>
