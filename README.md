...
Next.js (App Router) + **tRPC v11** + **Prisma / PostgreSQL**, TypeScript throughout. Client state in Zustand, server state via TanStack Query over tRPC; UI is shadcn/ui-style (Radix + Tailwind v4). A standalone WebSocket server (`server/ws-server.ts`) powers live logs and the container terminal; Redis for caching.
