...
# Production Deployment Setup Script for BuildMyStack
...
deploy_application() {
    echo -e "${BOLD}Deploying BuildMyStack Application...${NC}"
    ...
    spec:
      ...
      - name: app
        ...
        - name: NODE_ENV
          value: "production"
        - name: DATABASE_URL
          value: "postgresql://buildmystack:password@postgres-service:5432/buildmystack_prod"
        - name: REDIS_URL
...
data:
  prometheus.yml: |
    ...
    - job_name: 'buildmystack'
      static_configs:
      - targets: ['buildmystack-service:80']
